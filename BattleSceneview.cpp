...
QPixmap BattleSceneView::spriteFor(Creature* creature, int& targetHeight) const {
    ...
    if (key.contains("bomb")) {
        // 新图已处理为透明 PNG；MaskShape 会让点击、悬停和选中描边按角色轮廓响应。
        // 第 6 关 VolatileBomb 使用专属精修图，避免影响第 3 关 Bomb 的原风格参考。
        if (key.contains("volatilebomb") || (m_level && m_level->currentLevel() == 6)) {
            path = ":/assets/player_bomb_l6.png";
            targetHeight = 88;
        } else {
        ...
        // 先匹配各关卡的专属角色贴图（要放在通用 boss/goblin 判定之前，
        // 因为有些 Boss 的名字里也含有 "boss"）。
        if (key.contains("dummy") || key.contains("trainingdummy")) {
            path = ":/assets/enemy_dummy.png"; targetHeight = 218;
        } else if (key.contains("mimic")) {
            // 第 7 关：拟态魔复用石巨人立绘（伪装成场景物的怪物）。
            path = ":/assets/enemy_stone.png"; targetHeight = 216;
        } else if (key.contains("appraiser")) {
            // 第 7 关：鉴定师复用命匣/宝珠立绘（鉴定用的水晶）。
            path = ":/assets/prop_gem.png"; targetHeight = 150;
        } else if (key.contains("decoy")) {
            // 第 7 关：诱饵箱子复用金库立绘（看着像个结实的箱子）。
            path = ":/assets/enemy_vault.png"; targetHeight = 236;
        } else if (key.contains("berserker")) {
            path = ":/assets/enemy_berserker.png"; targetHeight = 210;
        } else if (key.contains("paladin")) {
            path = ":/assets/enemy_paladin.png"; targetHeight = 250;
        } else if (key.contains("lich")) {
            path = ":/assets/enemy_lich.png"; targetHeight = 248;
        } else if (key.contains("cultist")) {
            path = ":/assets/enemy_cultist.png"; targetHeight = 205;
        } else if (key.contains("king")) {
            path = ":/assets/enemy_king.png"; targetHeight = 246;
        } else if (key.contains("throne")) {
            path = ":/assets/prop_throne.png"; targetHeight = 240;
        } else if (key.contains("vault")) {
            path = ":/assets/enemy_vault.png"; targetHeight = 236;
        } else if (key.contains("saboteur")) {
            path = ":/assets/enemy_saboteur.png"; targetHeight = 216;
        } else if (key.contains("rogue")) {
            path = ":/assets/enemy_rogue.png"; targetHeight = 206;
        } else if (key.contains("gem")) {
            path = ":/assets/prop_gem.png"; targetHeight = 150;
        } else if (key.contains("stone")) {
            path = ":/assets/enemy_stone.png"; targetHeight = 216;
        } else if (key.contains("boss") || key.contains("door")) {
            // ShieldedBoss 是第 6 关角色，使用本次新增精修图。
            // DarkKnight / Elite 属于第 2、3 关，继续使用原 enemy_boss.png。
            path = ":/assets/enemy_boss_l6.png";
            targetHeight = 246;
        } else if (key.contains("darkknight") || key.contains("elite")) {
