...
QString Creature::baseClassCodeHtml(int level) {
    const bool attackPublic = (level == 4);  // 第四关：attack 放到 public 下，演示继承权限
    const bool showOnHit = (level == 6);      // 第六关：基类给出 virtual onHit 作为分发入口
    QString html;
    html += "<pre>";
...
QString Creature::classCodeHtml() const {
    ...
    const bool hasPublicMembers = !m_methods.isEmpty() || !m_classDeclarations.isEmpty();
    if (hasPublicMembers || !m_classBodyNote.isEmpty()) {
        if (hasPublicMembers) {
            html += line("public:");
        }
