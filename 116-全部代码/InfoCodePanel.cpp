...
static QString codePre(const QString& code) {
    return QString("<pre>%1</pre>").arg(code.toHtmlEscaped());
}

static QString bombClassCodeHtml() {
    return codePre(
...
InfoCodePanel::InfoCodePanel(QWidget* parent)
    ...
    m_shieldLabel = new QLabel("🛡 0");
    m_atkLabel = new QLabel("ATK -");
    m_defLabel = new QLabel("DEF -");
    ...
    m_statCard->setFixedHeight(156);

    m_hpBar->setObjectName("hpBar");
    m_shieldLabel->setObjectName("shieldBadge");
    ...
    m_shieldLabel->setAlignment(Qt::AlignCenter);
    m_atkLabel->setStyleSheet("color:#17234D; font-size:20px; font-weight:900; background:transparent; border:0; padding:0;");
    m_defLabel->setStyleSheet("color:#17234D; font-size:20px; font-weight:900; background:transparent; border:0; padding:0;");
    ...
    hpRow->setContentsMargins(0, 0, 0, 0);
    hpRow->setSpacing(8);
    hpRow->addWidget(m_hpBar, 1);
    hpRow->addWidget(m_shieldLabel, 0);
    ...
    statGrid->addLayout(hpRow, 0, 1);
    statGrid->addWidget(makeSmallTitle("ATK"), 1, 0);
    statGrid->addWidget(m_atkLabel, 1, 1);
    m_defTitleLabel = makeSmallTitle("DEF");
    statGrid->addWidget(m_defTitleLabel, 2, 0);
    statGrid->addWidget(m_defLabel, 2, 1);
    statGrid->addWidget(makeSmallTitle("阵营"), 3, 0);
    statGrid->addWidget(m_campLabel, 3, 1);
...
void InfoCodePanel::setClassChoices(const QList<Creature*>& creatures, int level) {
    ...
    if (level == 3) {
        m_classCodeByName.insert("Player::Bomb", bombClassCodeHtml());
        classNames << "Player::Bomb";
    }
...
void InfoCodePanel::showCreature(Creature* creature) {
    ...
    const bool showDef = creature->id() == "player";
    if (m_defTitleLabel) m_defTitleLabel->setVisible(showDef);
    m_defLabel->setVisible(showDef);
    m_defLabel->setText(QString::number(creature->def()));
    const bool showShield = creature->id() == "player" || creature->shield() > 0;
    m_shieldLabel->setVisible(showShield);
    m_shieldLabel->setText(QString("🛡 %1").arg(creature->shield()));
    m_shieldLabel->setStyleSheet(QString(
        "QLabel#shieldBadge {"
        " background:%1;"
        " color:%2;"
        " border:2px solid %3;"
        ...
        " min-width:54px;"
        "}"
    ).arg(creature->shield() > 0 ? "#D8EEFF" : "#EEF2F9")
     .arg(creature->shield() > 0 ? "#245AA4" : "#8391AB")
     .arg(creature->shield() > 0 ? "#86B8F2" : "#CAD5E7"));
...
void InfoCodePanel::showEmpty() {
    ...
    m_hpBar->setFormat("- / -");
    m_hpBar->setStyleSheet(hpBarStyle("#CBD5EA"));
    if (m_defTitleLabel) m_defTitleLabel->setVisible(true);
    m_defLabel->setVisible(true);
    m_shieldLabel->setVisible(true);
    m_shieldLabel->setText("🛡 -");
    m_shieldLabel->setStyleSheet(
        "QLabel#shieldBadge {"
        " background:#EEF2F9;"
        " color:#8391AB;"
        " border:2px solid #CAD5E7;"
        " border-radius:14px;"
        " font-size:16px;"
        " font-weight:900;"
        " padding:2px 10px;"
